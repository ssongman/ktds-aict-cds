...
- **Azure DNS Private Resolver = DNS 요청을 주고받는 Proxy (Region 리소스)**  onPrem 과 Azure사이 브리지 역할
  ...
  - 즉, Korea Centeral Region의 Resolver는 다른 리전에서 사용할수 없음.
...
1. **Access Key 제공**
   - 보안상 위험 (Storage Account 전체 권한 노출)
   - Key 탈취 시 큰 보안사고 위험 → ❌
2. **Azure Files + SAS 제공 (클라이언트 직접 접근)**
   - SAS는 권한/만료 시간 제한 가능
   - 클라이언트가 **WAS 경유하지 않고 직접 Azure Files에 접근**하므로 WAS 부하 감소
   - 보안+비용 효율 모두 충족 → ⭕
3. **Azure Shared Disk 사용**
   - Shared Disk는 VM 간 클러스터링/Failover 목적이지, 대규모 파일 공유/직접 다운로드용이 아님
   - 클라이언트 직접 접근 문제 해결 불가 → ❌
4. **Azure Blob Storage + SAS 제공 (클라이언트 직접 접근)**
   - Blob은 Files보다 대용량 처리·비용 효율이 더 우수
   - 특히 동영상, 분석용 파일 등 **Object Storage에 적합**
   - SAS 제공으로 보안·부하 문제 해결 → ⭕ (Files보다 더 좋은 선택일 수 있음)
5. **Azure Function을 통해 업/다운로드**
   - WAS 대신 Function이 중계하지만 여전히 **서버가 중간 Proxy** 역할
   - 불필요한 비용·지연 증가 → ❌
...
1번 : Premium SKU면 **Global Reach** 기능을 통해 타 리전(VNet) 게이트웨이와 연결해 통신 가능.

2번 : **Standard SKU**는 글로벌은 안되지만, 같은 **페어 Region** 안에서는 지원가능 (Korea Central ↔ Korea South 는 **페어 Region** 관계)

3번 : **ExpressRoute는 기본적으로 On-Prem ↔ Azure 간 전용선** 이므로 서로 다른 **On-Prem ↔ On-Prem 간 통신**은 기본 상태에선 **불가능**

4번 : ExpressRoute Circuit/ER Gateway 구간에 QoS 없음. 여러 VNet이 한 회선 대역폭을 공유하므로 설계 시 분리/대역폭 계획 필요.

5번 : ER 회선 자체가 VNet-to-VNet 라우팅을 제공하진 않음(일반적으로 **VNet Peering** 필요)   좀 애매함.



### **QoS (Quality of Service)**

- **네트워크 트래픽 품질 보장 기능**을 의미함
- 특정 트래픽(예: 음성, 영상, 금융 거래 등)에 대해 **우선순위(priority)** 또는 **대역폭 보장(bandwidth guarantee)**을 설정하여, 다른 일반 트래픽에 의해 지연·손실되지 않도록 보장하는 기술
- 4번 보기에서 **QoS 없음**이란, ExpressRoute Circuit 내부에서 특정 VNet 트래픽에 대역폭을 보장해주는 기능이 없으므로 **설계 단계에서 트래픽 경합을 고려해야 한다**는 의미
...
Kubernetes가 Deployment 업데이트나 Scale-in으로 **Pod를 종료**시킬 때 순서:

1. **kubectl delete pod** 또는 Deployment 롤링 업데이트 시작
2. Kubelet이 종료 요청을 받음 → 해당 Pod에 SIGTERM 시그널 전달
3. Pod의 **TerminationGracePeriodSeconds** (기본 30초) 동안 프로세스가 정상 종료하도록 기다림
4. 종료되지 않으면 SIGKILL 로 강제 종료

하지만 아래와 같은 문제가...

Pod가 종료 요청을 받더라도 **트래픽이 해당 Pod로 라우팅될 수 있음** → 종료 중간에 들어온 요청은 실패 (500/timeout 발생).



### PreStop Hook

Pod 스펙(spec.containers.lifecycle)에 정의할 수 있는 **라이프사이클 훅** 중 하나.

- Pod 종료(SIGTERM) 직전에 **지정된 명령/HTTP 호출**을 실행할 수 있음.
- PreStop 실행이 끝날 때까지 Kubernetes는 종료를 계속 진행하지 않음.
...
* Pod가 종료될 때 10초 대기 → 이 시간 동안 Readiness가 false가 되어 LB 트래픽에서 제외됨.
...
**1번** : Redis는 가장 대표적인 캐시 계층이며 DB 조회 부하를 줄이는 데 적합

**2번** : ASP.NET, Spring 등에서 Redis를 세션 저장소(Session Store)로 사용하는 것은 일반적인 활용 예시
...
* "Ubuntu"는 **노드 OS**일 뿐이고, 컨테이너 베이스 이미지는 Ubuntu에 **제한되지 않음**.
* 리눅스 컨테이너는 다양한 베이스 이미지를 사용할 수 있고, 윈도우 컨테이너는 **별도 Windows 노드풀**이 필요
...
* 해당 Node Pool 준비되면 Pod 들을 스케쥴링 및 최소한의 서비스 검증후
* 기종 Node Pool Pod delete 및 Node Pool 삭제
